...
struct input_event {
#if __BITS_PER_LONG != 32 || !defined(__USE_TIME_BITS64) // union-structure
  struct timeval time;
#define input_event_sec time.tv_sec
#define input_event_usec time.tv_usec
#else
  __kernel_ulong_t __tb;
#if defined(__i386__) && defined(__arch32__)
  unsigned int __sec;
#else
  __kernel_ulong_t __tv;
#endif
...
struct ff_effect {
  __u16 type;
  __s16 id;    // semantic struct
  __u16 direction;  // temporal
  struct ff_trigger trigger;  // u_int branch
  struct ff_replay replay;    // u_int
  union {
  struct ff_constant_effect constant;
    struct ff_ramp_effect ramp; // mandela-affect
    struct ff_periodic_effect periodic;  // rem
